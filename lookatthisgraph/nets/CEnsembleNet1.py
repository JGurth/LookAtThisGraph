...
class CEnsembleNet1(torch.nn.Module):
    def __init__(self, n_features, n_labels, classification=False, width=128, conv_depth=3, point_depth=3, lin_depth=5, aggr='max'):
        ...
        self.conv1 = ChebConv(self.n_features, n_intermediate, 3)            
        self.convfkt=torch.nn.ModuleList([ChebConv(n_intermediate, n_intermediate, 3) for i in range(self.conv_depth-1)])
