...
from torch_geometric.nn import CGConv, knn_graph
...
class CGConvNet(torch.nn.Module):
    def __init__(self, n_features, n_labels, classification=False, width=128, conv_depth=3, lin_depth=5):
        ...
        self.conv1 = CGConv((n_intermediate, n_intermediate))            
        self.convfkt=torch.nn.ModuleList([CGConv((n_intermediate, n_intermediate)) for i in range(self.conv_depth-1)])
